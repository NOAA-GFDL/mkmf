...
Pronunciation
-------------
__mkmf__ is pronounced make-make-file or make-m-f or even McMuff (Paul Kushner's suggestion).
...
* __-o otherflags__ is a list of compiler directives to be passed to
  ...
  invoked on .F and .F90 files; __otherflags__ apply to all source files
  (.f and .f90 as well).
...
* __-f__ is a formatting flag to restrict lines in the makefile to 256
  characters.
* __-m makefile__ is the name of the makefile written (default _Makefile_)
...
This can contain the cpp `#defines` which may change from run to
run.  cpp options that do not change between compilations should be
placed in $(CPPFLAGS).
...
the arguments to __mkmf__, otherwise, the loader is likely to complain.
...
3. If an argument is a regular file, it is presumed to contain a list
   of sourcefiles.  Any line not containing a sourcefile is
...
`.cppdefs`.  If the argument to __-c__ is changed in a subsequent call:
...
use &
   this_module
```
it would confuse __mkmf__. Similarly, a Fortran include statement must not be split across lines.
...
when cpp `#includes` buried within obsolete `#ifdefs` ask for files that
don't exist:
```fortran
#ifdef obsolete
